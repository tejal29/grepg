...
from util.utils import since_time_in_words, print_util
from lib.cheat_sheets_model import CheatSheetsCache
...
class GrepgClient(object):
  def __init__(self, username, topic, search_term, colorize, force, ttl, base_url=None, access_token=None, ):
  ...
  def _get_user_topics(self):
    with CheatSheetsCache(self.ttl) as cs:
      if self.force or not cs.has_valid_user_cheat_sheets(self.user_name):
        ...
        cs.add_or_update_sheet(self.user_name, user_sheet)
  ...
  def get_user_topics(self, print_header=True):
    ...
    if print_header:
      print_util("User: {0}, Last Fetched: {1} ago".format(self.user_name, since_time_in_words(since_timestamp)), 'green', self.colorize)
  ...
  def list_user_topic_cheats(self):
    print_util('User: {0}, Topic: {1}'.format(self.user_name, self.topic), 'green', self.colorize)
  ...
  def search_user_topic_cheats(self):
    ...
    search_results = filter(lambda topic_cheat: self.search_term in topic_cheat.description, user_topic_cheats)
    if search_results:
